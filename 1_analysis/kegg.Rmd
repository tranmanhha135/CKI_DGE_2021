...
big_df[[x]] <- enrichKEGG_treat[[x]][1:10,]
}

#combine all list from big df into one
df <- as.data.frame(do.call(rbind, lapply(big_df, as.data.frame))) %>% rownames_to_column("group") %>% dplyr::select(-c("BgRatio", "pvalue", "geneID")) 
...
df$group <- gsub(pattern = "\\.hsa[0-9]*", "", df$group)
...
ggsave(filename = "enrichedKEGG_treat.svg", width = 7, height = 6.5, plot = last_plot(), path = here::here("2_plots/4_kegg/"))
...
## Heatmap of `r kegg_pathway_name`

### Partial 

```{r partialPathway, fig.width=7, fig.height=7}
# kegg_id <- "hsa04657"
# kegg_pathway <- KEGGREST::keggGet(kegg_id)
# kegg_pathway_name <- kegg_pathway[[1]]$NAME
